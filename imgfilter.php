<?php
$confdir= array("all","drop","ok");//modify the folder name if you want to use another folder name.
global $basedir;
list($basedir,$dropdir,$okdir) = $confdir;
global $dataimg;
$dataimg=json_decode(file_get_contents("all.json"),TRUE);//read generated file list.
session_start();//start browser session
function GetFileList($f){//read all images and write out the filename to the json file.
    $file = scandir("./".$f."/");
    array_shift($file);
    array_shift($file);
...
function R18($imgid)
{
    ...
    {
        global $r18del;
        $r18del="File was moved to unwanted folder.";
...
function Safe($imgid)
{
    ...
    if(Movefile($filepath,"ok",$GLOBALS['dataimg'][$imgid])){
        global $safe;
        $safe="File was moved to wanted folder.";
        $nowimg=$_SESSION['now_img'];
        if($_SESSION['now_img']==$GLOBALS['allimgcount'])
        {
        $_SESSION['now_img']=0;  
...
if($_GET['init'])
{
    GetFileList("all");//modify this if you modify the confdir
    $_SESSION['now_img']=0;
    global $initstat;
    $initstat="Database has been initialized.";
}
...
<img src=".$basedir."/".rawurlencode($dataimg[$_SESSION['now_img']])."></img>");
    ...
    echo ("<div class=\"main\"><span class=\"indicator\">Image".($_SESSION['now_img']+1)."/".($allimgcount+1)."</span><p class=\"hint\">This image was moved to other folders.</p>");
}
if($_SESSION['now_img']==0)
{
    echo ("<a href=\"?img=next\" class=\"next\" title=\"Next\">&gt;</a>
    <a href=\"?act=18\" class=\"decline\" title=\"Decline\">N</a>
    <a href=\"?act=ok\" class=\"accept\" title=\"Accept\">Y</a>
    <a href=\"?init=true\" class=\"init\">Database Init</a>
    ");
}
else if($_SESSION['now_img']==$allimgcount)
{
    echo ("<a href=\"?img=prev\" class=\"previous\" title=\"Prev\">&lt;</a>
    <a href=\"?act=18\" class=\"decline\" title=\"Decline\">N</a>
    <a href=\"?act=ok\" class=\"accept\" title=\"Accept\">Y</a>
    <a href=\"?init=true\" class=\"init\">Database Init</a>");
}
    ...
    echo ("<a href=\"?img=prev\" class=\"previous\" title=\"Prev\">&lt;</a>
    <a href=\"?img=next\" class=\"next\" title=\"Next\">&gt;</a>
    <a href=\"?act=18\" class=\"decline\" title=\"Decline\">N</a>
    <a href=\"?act=ok\" class=\"accept\" title=\"Accept\">Y</a>
    <a href=\"?init=true\" class=\"init\">Database Init</a>");
}
//echo ("<a href=\"?init=true\">Database Ini</a><br>");
